...
import { Calendar, MapPin, Users, DollarSign, Clock, FileText, User } from "lucide-react";
    ...
    try {
      ...
      if (refMatch) {
        ...
        if (!error && data) {
          ...
          // Fetch user profile for this event
          if (data.user_id) {
        ...
        if (isEventRelated) {
          // Get the most recent event as a fallback for admin
          const { data, error } = await supabase
            .from("events")
            .select("*")
